...
import com.minu.player.netflix.model.AudioTrack
...
import com.minu.player.netflix.model.TextTrack
import com.minu.player.netflix.model.VideoStream
import com.minu.player.netflix.model.VideoTrack
...
val REPLACE_MACRO_LIST = listOf("nb", "nn")
val REPLACE_MACRO_LANG = mapOf("no" to listOf("nb", "nn"))
val LOCALE_CONV_TABLE = mapOf("es-ES" to "es-Spain", "pt-BR" to "pt-Brazil", "fr-CA" to "fr-Canada", "ar-EG" to "ar-Egypt", "nl-BE" to "nl-Belgium", "en-GB" to "en-UnitedKingdom")

fun applyLangCodeChanges(dataList: List<LanguageTrack>) {
    ...
    for (item in dataList) {
        ...
        convertMacroLanguages(item, langList)
        fixLocalLanguages(item)
    }
}

fun convertMacroLanguages(item: LanguageTrack, langList: ArrayList<String>) {
    if (REPLACE_MACRO_LANG.contains(item.language)) {
        var mainLang = ""
        for ((key, value) in REPLACE_MACRO_LANG) {
            if (value.contains(item.language)) {
                mainLang = key
            }
        }
        if (!langList.contains(mainLang)) {
            item.language = mainLang
        }
    }
}

fun fixLocalLanguages(item: LanguageTrack) {
    if (item.language == "pt-BR") {
        item.language = "pb"
    }
    if (item.language.length > 2) {
        if (LOCALE_CONV_TABLE.keys.contains(item.language)) {
            item.language = LOCALE_CONV_TABLE[item.language]!!
        } else {
            Log.d("custom error", "fix_locale_languages: missing mapping conversion for locale " + item.language)
        }
    }
}
...
fun getIdDefaultAudioTrack(manifest: Manifest): String {
    ...
    if (audioLanguage == "mediadefault") {
        ...
        val profileLanguageCode = PreferenceManager.getDefaultSharedPreferences(appContext).getString(
